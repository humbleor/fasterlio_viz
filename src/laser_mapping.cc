...
void LaserMapping::SubAndPubToROS(ros::NodeHandle &nh) {
    ...
    pub_trunk_ = nh.advertise<visualization_msgs::MarkerArray>("/trunk_array", 10000);
}
...
LaserMapping::LaserMapping(double accumulation_time, double max_distance) {
    preprocess_.reset(new PointCloudPreprocess());
    p_imu_.reset(new ImuProcess());
    cylinder_fitter_.reset(new TreeCylinderFitter(accumulation_time, max_distance));
}
...
// int accumulation_time = 10;
// double max_distance = 5.0;
// TreeCylinderFitter fitter(accumulation_time, max_distance);
void LaserMapping::PublishTrunk(const ros::Publisher &pub_trunk) {
    // PointCloudType::Ptr laserCloudWorld;
    // if (dense_pub_en_) {
    //     PointCloudType::Ptr laserCloudFullRes(scan_undistort_);
    //     int size = laserCloudFullRes->points.size();
...
void LaserMapping::Savetrunk(const std::string &trunk_file) {
    ...
    std::vector<TrunkCylinderDesc> cylinders = cylinder_fitter_->getAllTrunkCylinderDesc();
    ofs << "#trunk_id (position.x, position.y, position.z) (direction.x, direction.y, direction.z) radius height" <<std::endl;
    for(const auto& cylinder : cylinders) {
        ofs << cylinder.trunk_id
